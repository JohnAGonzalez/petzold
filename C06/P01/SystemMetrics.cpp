...
int WINAPI WinMain(
	...
	int iCmdShow)
{
	static TCHAR	szAppName[] = TEXT("SystemMetrics4");
	HWND			hWnd;
	MSG				msg;
	WNDCLASS		wndClass;
	...
	hWnd = CreateWindow(
		szAppName, TEXT("Get System Metrics No. 4"), WS_OVERLAPPEDWINDOW,
	...
	LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		// set horizontal scrol bar range and page size
		si.cbSize = sizeof(si);
		si.fMask = SIF_RANGE | SIF_PAGE;
		si.nMin = 0;
	...
	case WM_HSCROLL:
		...
		if (si.nPos != iHorzPos)
		{
			ScrollWindow(hWnd, 0, cxChar * (iHorzPos - si.nPos), NULL, NULL);
			UpdateWindow(hWnd);
		}
		return 0;

	case WM_KEYDOWN:
		// Map keystrokes to scroll bar movements
		switch (wParam)
		{
